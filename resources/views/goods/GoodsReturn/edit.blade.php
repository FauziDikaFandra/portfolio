...
<div class="starter-template mt-3  shadow p-3 mb-5 bg-white rounded">
    ...
    <form action="{{url('/goods/goods-return/change')}}" method="post">

        <div class="form-row">
            <div class="form-group col-md-4">
                <label for="gr_code">RT Code</label>
                <input type="text" class="form-control" id="rt_code" placeholder="GR Code" name="rt_code" value="{{$goodsreturn[0]->rt_code}}" readonly>
            </div>
            <div class="form-group col-md-4">
                <label for="status">Status</label>
                <select id="status" name="status" class="form-control">
                    @if ($goodsreturn[0]->status === 'POST')
                    <option selected>POST</option>
                    <option>OPEN</option>
                    <option>CLOSE</option>
                    <option>CANCEL</option>
                    @elseif ($goodsreturn[0]->status === 'OPEN')
                    <option>POST</option>
                    <option selected>OPEN</option>
                    <option>CLOSE</option>
                    <option>CANCEL</option>
                    @elseif ($goodsreturn[0]->status === 'CLOSE')
                    <option>POST</option>
                    <option>OPEN</option>
        ...
        <div class="form-row">
            <div class="form-group col-md-4">
                ...
                <input type="date" class="form-control" id="docdate" name="docdate" value="{{date('Y-m-d', strtotime($goodsreturn[0]->documentdate))}}">
            </div>
        ...
        <div class=" form-row">
            <div class="form-group col-md-10">
                ...
                <input type="text" class="form-control" id="remarks" name="remarks" placeholder="Remarks" value="{{$goodsreturn[0]->remarks}}">
            </div>
        </div>
        <input type="hidden" name="_token" value="<?php echo csrf_token() ?>">
        <button type=" submit" class="btn btn-primary">Update</button>
        <span class="mr-5" style="color: red;">{{$status ?? '' != '' ?  'Please Entry Return Detail First !!!' : ''}}</span>
    </form>
</div>

...
<div class="starter-template mt-3  shadow p-3 mb-2 bg-white rounded">
    <form action="{{url('/goods/goods-return-details/create')}}" method="post">
        <div class="form-row">
            <div class="form-group col-md-2">
                <label for="gr_code">RT Code</label>
                <input type="text" class="form-control" id="rt_code" name="rt_code" value="{{$rt_code}}" readonly>
            </div>
...
<div class="table-responsive mt-3  shadow p-3 mb-5 bg-white rounded">
    <table class="table table-striped table-sm">
        ...
        <tbody>
            <?php $line = 0;
            $total = 0; ?>
            @foreach ($goodsreturndetails ?? '' as $gr)
            <?php $line++;
            $total += $gr->qty ?>
            <tr>
                <td>{{$line}}</td>
                ...
                <td>
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                            <a class="dropdown-item" href="{{route('delete.detailRT',['id'=>$gr->rt_code,'article'=>$gr->article_code])}}"><span class="mr-2" data-feather="delete"></span>Delete</a>
                        </div>
                    </div>
                </td>
            </tr>
